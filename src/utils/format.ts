export const formatTokenValue = (
  ...
  const roundedFraction = fraction / 10n ** BigInt(Math.max(decimals - fractionDigits, 0));

  const formatted =
    roundedFraction === 0n ? `${integer}` : `${integer}.${toFixed(roundedFraction.toString(), fractionDigits)}`;
...
const toFixed = (number: string, digits: number) => {
  return trimEnd(number.padStart(digits, "0"), "0");
};

const trimEnd = (s: string, char: string) => {
