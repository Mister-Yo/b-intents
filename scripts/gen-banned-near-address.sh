...
if command -v jq &> /dev/null; then
    if jq '.' "$output_file" > "$output_file.tmp" && mv "$output_file.tmp" "$output_file"; then
        echo "File formatted successfully with jq"
    else
        echo "Warning: jq formatting failed, but JSON file was created"
        rm -f "$output_file.tmp"
    fi
else
    echo "Warning: jq not found, JSON file created without formatting"
fi
